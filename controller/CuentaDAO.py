...
class CuentaDAO:
    ...
    def consultarCuenta(self, numeroCuenta):
        ...
        cursor = cdb.cursor()
        cursor.execute(
            f"SELECT c.numero_cuenta, c.saldo, c.fecha_apertura, c.tasa_interes, c.ultimo_movimiento FROM cuenta AS c"
    ...
    def crearCuenta(self, numeroCuenta, idSucursal, saldo, fechaApertura, tasaInteres, ultimoMovimiento, idUsuario):
        try:
            ...
            cursor = cdb.cursor()
            cursor.execute(
                f"INSERT INTO cuenta (numero_cuenta, id_sucursal_cuenta, saldo, fecha_apertura, tasa_interes,"
                f" ultimo_movimiento, id_usuario_cuenta) VALUES ('{numeroCuenta}', '{idSucursal}', '{saldo}', '{fechaApertura}', '{tasaInteres}', '{ultimoMovimiento}', '{idUsuario}')")
            cdb.commit()
            return True
